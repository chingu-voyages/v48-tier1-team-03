...
// HELPER FUNCTIONS
function filterGrid(substr) {
  // clear the current HTML in the grid
  grid.innerHTML = "";

  // fill the grid container with the filter
  for (const entry of data) {
    if (entry.name.includes(substr)) {
...
function entryCard(entry) {
  ...
  containerEl.classList.add("col-12", "col-md-6", "col-lg-4", "col-xl-3");
  containerEl.setAttribute("data-index", entry.id);
